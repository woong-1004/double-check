...
export default function NewsCheckClientContent({
  ...
  const sortedInitialNews = [...initialGroupedNews].sort(
      ...
      const matchesKeywords =
        ...
        selectedKeywords.some(
          (keyword) =>
            ...
            group.items.some((item) => item.aiSummary.includes(keyword)) ||
            (group.commonFactCheck && group.commonFactCheck.includes(keyword))
        );
      ...
      {debouncedSearchTerm && filteredNews.length === 0 ? (
        ...
        <>
          ...
          {paginatedTopicGroups.map((group) => {
            ...
            return <NewsTopicDisplay key={topicDateKey} group={group} />;
          })}
          {renderPaginationControls()}
        </>
      )}
